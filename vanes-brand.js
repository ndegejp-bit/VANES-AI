...
(function(){
  ...
  if(modal){const card=modal.querySelector('.name-card');if(card){const oldP=card.querySelector('p:not(.eyebrow)');if(oldP)oldP.textContent='Tell VANES your name, education level and subjects so your dashboard can adapt to you.';if(!card.querySelector('.first-launch-note')){const note=document.createElement('div');note.className='first-launch-note';note.innerHTML='<strong>VANES = Verseversatile Adaptive Neuro Emergent System</strong><br>Study, analyse, practise, plan, create and learn with Tanzanian curriculum-aware AI.';card.appendChild(note);}}}
  ...
  const upgrade=document.createElement('script');upgrade.src='/vanes-product-upgrade.js?v=20260914c';document.body.appendChild(upgrade);
  upgrade.onload=()=>{const context=document.createElement('script');context.src='/vanes-ai-context-fix.js?v=20260914b';document.body.appendChild(context)};
})();
