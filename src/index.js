...
function toImage(value, mime = "image/png") {
  ...
  const md = s.match(/!\[[^\]]*\]\((https?:\/\/[^\s)]+|data:image\/[^)]+)\)/i); if (md) return md[1];
  const url = s.match(/https?:\/\/[^\s<>"')]+/i); if (url) return url[0];
  if (s.length > 100 && /^[A-Za-z0-9+/=_\-]+$/.test(s)) return `data:${mime};base64,${s}`;
  return null;
}
function collectImages(value, output = [], mime = "image/png", seen = new WeakSet()) {
  if (value == null) return output;
  if (typeof value === "string") {
    const trimmed = value.trim();
    try { if (/^[\[{]/.test(trimmed)) { const parsed = JSON.parse(trimmed); return collectImages(parsed, output, mime, seen); } } catch {}
    const image = toImage(value, mime); if (image) output.push(image); return output;
  }
...
async function handleChat(request, env) {
  ...
  const length = Number(request.headers.get("Content-Length") || 0); if (length > MAX_BODY) return json({ error: "Request is too large." }, 413, headers);
...
async function handleImage(request, env) {
  ...
  if (request.method !== "POST") return json({ error: "Method not allowed" }, 405, headers);
  if (!env.OPENROUTER_API_KEY) return json({ error: "The VANES AI server is not configured yet. Add the runtime secret OPENROUTER_API_KEY in Cloudflare." }, 500, headers);
  const length = Number(request.headers.get("Content-Length") || 0); if (length > MAX_BODY) return json({ error: "Image request is too large." }, 413, headers);
  let body; try { body = await request.json(); } catch { return json({ error: "Invalid JSON body." }, 400, headers); }
