...
function inject(){
  ...
  <article class="panel settings-card"><p class="eyebrow">OB TECH-LABS OPPORTUNITIES</p><h2>Interested in the field?</h2><label>Name<input id="fieldName" maxlength="80" required></label><label>Phone number<input id="fieldPhone" type="tel" maxlength="25" required></label><label>Email<input id="fieldEmail" type="email" maxlength="120" required></label><button class="secondary-button" id="fieldButton">Contact OB Tech-Labs</button><div id="fieldStatus" class="settings-status"></div></article>
...
function wire(){
 ...
 document.getElementById('clearLocalData').onclick=()=>{if(confirm('Clear local VANES data on this device?')){['vanes-user-name','vanes-learner-profile-v1','vanes-chat-conversations-v1','vanes-study-tracking-v2','vanes-study-summaries-v1','vanes-daily-streak-v2','vanes-settings-v1','vanes-theme','vanes-notification-settings-v1'].forEach(k=>localStorage.removeItem(k));location.reload()}};
 ...
 document.getElementById('fieldButton').onclick=()=>send('field',{name:document.getElementById('fieldName').value.trim(),phone:document.getElementById('fieldPhone').value.trim(),email:document.getElementById('fieldEmail').value.trim()},document.getElementById('fieldStatus'));
